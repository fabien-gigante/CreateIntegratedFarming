...
public class HeatResistantFishingNetMovementBehaviour implements MovementBehaviour {
    @Override
    ...
    public void visitNewPosition(MovementContext context, BlockPos pos) {
        if (context.world instanceof ServerLevel level) {
            var fishing = getFishingNetContext(context, level);
            var inLava = fishing.visitNewPositon(level, pos);
            if (!inLava || fishing.timeUntilCatch > 0)
                return;
            if (fishing.canCatch()) {
                var params = fishing.buildLootContext(context, level, pos);
                LootTable lootTable;
                if (context.world.dimension() == Level.NETHER)
                    lootTable = context.world.getServer().reloadableRegistries().getLootTable(NDULootTables.NETHER_FISHING);
                else
                    lootTable = context.world.getServer().reloadableRegistries().getLootTable(NDULootTables.LAVA_FISHING);
    ...
    protected HeatResistantFishingNetContext getFishingNetContext(MovementContext context, ServerLevel level) {
        if (!(context.temporaryData instanceof HeatResistantFishingNetContext)) {
            context.temporaryData = new HeatResistantFishingNetContext(level, new ItemStack(NDUItems.LAVA_FISHING_ROD.asItem()));
        }
        return (HeatResistantFishingNetContext) context.temporaryData;
    }
}
